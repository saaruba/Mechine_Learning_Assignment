"""
Visualize a trained PPO model on MuJoCo Reacher.
...
4. Loads a PPO model and runs evaluation episodes deterministically.

How to run from Task-2:
python src/evaluator/visualize_ppo.py --model-path results/ppo_seed7/models/ppo_reacher_pixels_seed_7.zip --episodes 5 --seed 7 --sleep 0.05
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize PPO on pixel-based MuJoCo Reacher.")
...
def main() -> None:
    args = parse_args()
    model_path = Path(args.model-path if False else args.model_path).expanduser().resolve()
    ...
    print("=" * 70)
    print("PPO Reacher Visualization")
