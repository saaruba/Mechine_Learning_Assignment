"""
Debug TD3 training script for MuJoCo Reacher with image observations.
...
3. Trains a TD3 agent with a CNN policy for a short debug run.
...
# Task-2/src/training/train_td3.py
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory to save model/logs/metadata. Default: Task-2/results/td3_debug",
    )
...
def main() -> None:
    ...
    print("=" * 70)
    print("Starting TD3 debug training (image-based Reacher)")
    ...
    try:
        ...
        print(f"[INFO] TD3 hyperparameters: {td3_hyperparams}")

        # Conservative TD3 defaults for image-based off-policy training.
        model = TD3(
            policy="CnnPolicy",
            env=vec_env,
            verbose=1,
            ...
            device=td3_hyperparams["device"],
            buffer_size=td3_hyperparams["buffer_size"],
            learning_starts=td3_hyperparams["learning_starts"],
            batch_size=td3_hyperparams["batch_size"],
            learning_rate=td3_hyperparams["learning_rate"],
        )
        ...
        print("[INFO] Debug TD3 training run finished successfully.")
    finally:
