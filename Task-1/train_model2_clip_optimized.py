"""
Training Script for Model 2: CLIP-Optimized
Enhanced training configuration for better accuracy on medical VQA
"""
...
from config_models_2_3_fast import TRAINING_CONFIG_CLIP, DATA_CONFIG, OUTPUT_CONFIG
from model_clip_fast import create_clip_fast_model
...
class SLAKEDataset(Dataset):
    """SLAKE dataset for CLIP model with augmentation"""
...
class CLIPTrainer:
    """Trainer for CLIP model with enhanced optimization"""
    ...
    def __init__(self, model, train_loader, val_loader, test_loader, num_classes, class_names, device='cuda'):
        ...
        # Enhanced optimizer with different LRs for different components
        self.optimizer = optim.AdamW([
            {'params': self.model.clip_model.text_model.parameters(), 'lr': 1e-4},
    ...
    def train_epoch(self, epoch):
        ...
        pbar = tqdm(self.train_loader, desc=f"Model 2 Epoch {epoch} - Train")
        for batch in pbar:
            ...
            logits, _ = self.model(images, questions)
    ...
    def validate(self):
        ...
        pbar = tqdm(self.val_loader, desc=f"Model 2 Epoch - Val")
        with torch.no_grad():
            for batch in pbar:
                images = batch['image'].to(self.device)
                questions = batch['question']
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if val_metrics['accuracy'] > self.best_val_acc:
                ...
                checkpoint_path = os.path.join(
                    ...
                    f"model2_clip_epoch_{epoch}_acc_{val_metrics['accuracy']:.4f}.pt"
                )
    ...
    def test(self):
        ...
        pbar = tqdm(self.test_loader, desc="Model 2 Testing")
        with torch.no_grad():
            for batch in pbar:
                images = batch['image'].to(self.device)
                questions = batch['question']
...
def main():
    ...
    print("""
============================================================
MODEL 2: CLIP-OPTIMIZED TRAINING
...
============================================================
    ...
    train_dataset = SLAKEDataset(train_data, dataset_path, answer_vocab, split='train')
    val_dataset = SLAKEDataset(val_data, dataset_path, answer_vocab, split='val')
    test_dataset = SLAKEDataset(test_data, dataset_path, answer_vocab, split='test')
    ...
    # Create model
    logger.info("")
    logger.info("Creating CLIP-Optimized model...")
    model = create_clip_fast_model(num_classes=num_classes, device=device)

    # Print model info
    model_info = model.get_model_info()
    print("\n" + "="*60)
    print("MODEL 2: CLIP-OPTIMIZED ARCHITECTURE")
    print("="*60)
    ...
    trainer = CLIPTrainer(model, train_loader, val_loader, test_loader, num_classes, list(answer_vocab.keys()), device=device)
    ...
    results_path = os.path.join(output_dir, 'model2_clip_optimized_results.json')
    ...
    cm_path = os.path.join(output_dir, 'model2_clip_optimized_confusion_matrix.png')
    ...
    calibration_path = os.path.join(output_dir, 'model2_clip_optimized_calibration.png')
    ...
    logger.info("")
    logger.info("Model 2 (CLIP-Optimized) training complete!")
