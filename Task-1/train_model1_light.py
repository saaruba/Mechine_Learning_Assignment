"""
Training Script for Model 1 LIGHT: MobileNet + DistilBERT
Optimized for speed: 3-5 hours instead of 20 hours
Restart from scratch (no BioBERT connection issues!)
"""
...
from model_cnn_distilbert_light import create_light_model
...
class SLAKEDataset(Dataset):
    """SLAKE dataset for lightweight model"""
...
class LightTrainer:
    ...
    def __init__(self, model, train_loader, val_loader, test_loader, num_classes, class_names, device='cuda'):
        ...
        # FAST OPTIMIZER: AdamW with lower learning rate
        self.optimizer = optim.AdamW(
            model.parameters(),
            lr=5e-5,  # Slightly lower for stability
            weight_decay=1e-5
        )
        ...
        self.patience = 3  # FAST: Early stop after 3 epochs no improvement
        self.patience_counter = 0

        logger.info("✓ Light Trainer initialized")
    ...
    def train_epoch(self, epoch):
        ...
        pbar = tqdm(self.train_loader, desc=f"Model 1-LIGHT Epoch {epoch} - Train")
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            # Tokenize questions on-the-fly
            from transformers import AutoTokenizer
            tokenizer = AutoTokenizer.from_pretrained("distilbert-base-uncased")
            tokens = tokenizer(questions, max_length=64, padding='max_length',
    ...
    def validate(self, epoch):
        ...
        pbar = tqdm(self.val_loader, desc=f"Model 1-LIGHT Epoch {epoch} - Val")
    ...
    def test(self):
        ...
        pbar = tqdm(self.test_loader, desc="Model 1-LIGHT Testing")
    ...
    def save_checkpoint(self, epoch, metrics):
        ...
        checkpoint_path = os.path.join(
            ...
            f'model1_light_epoch_{epoch}_acc_{metrics["accuracy"]:.4f}.pt'
        )
    ...
    def load_best_checkpoint(self):
        ...
        checkpoints = sorted([f for f in os.listdir(OUTPUT_CONFIG['checkpoint_dir'])
                            if 'model1_light' in f and f.endswith('.pt')])
        if checkpoints:
...
def main():
    logger.info("="*60)
    logger.info("MODEL 1 LIGHT: MobileNet + DistilBERT (FAST VERSION)")
    logger.info("="*60)
    ...
    model = create_light_model(num_classes=num_classes, device=device)
    model_info = model.get_model_info()

    print("\n" + "="*60)
    print("MODEL 1 LIGHT: ARCHITECTURE")
    print("="*60)
    ...
    # OPTIMIZED: 15 epochs with early stopping for better convergence
    ...
    # Save results
    results = {
        'model_name': 'Model-1-Light (MobileNet + DistilBERT)',
        'timestamp': datetime.now().isoformat(),
        'training_history': history,
        'test_metrics': test_metrics,
    }

    results_path = os.path.join(OUTPUT_CONFIG['result_dir'], 'model1_light_results.json')
    trainer.metrics_calculator.save_metrics(results, results_path)

    # Visualizations
    logger.info("\nGenerating visualizations...")
    confusion_path = os.path.join(OUTPUT_CONFIG['result_dir'], 'model1_light_confusion_matrix.png')
    trainer.metrics_calculator.plot_confusion_matrix(test_labels, test_preds, confusion_path)

    calibration_path = os.path.join(OUTPUT_CONFIG['result_dir'], 'model1_light_calibration.png')
    trainer.metrics_calculator.plot_calibration_curve(test_labels, test_proba, calibration_path)

    logger.info("\n✓ Model 1 LIGHT training complete!")
